...
ai_y=pos_ai[0];
   ...
   */
    ...
    public static int[] VictoryIn1Step(char sym){
        int[]best_step={-1,-1};
        ...
        for(int i=0; i<SIZE_X;i++) {
            ...
            for (int j = 0; j < SIZE_Y;j++ ) {
                ...
                while(buff_i<SIZE_X&&buff_j<SIZE_Y){
                    ...
                    if(curr==count-2&&buff_j+2<SIZE_Y&&buff_i+2<SIZE_X&&
                            field[buff_j+2][buff_i+2]==sym&&field[buff_j+1][buff_i+1]==EMPTY_DOT)
                    {best_step=new int[]{buff_j+1,buff_i+1};return best_step;}//вариант xxx.x
                    buff_i++;
                    buff_j++;
                }
            }
                ...
                while(buff_i>-1&&buff_j<SIZE_Y){
                    if(field[buff_j][buff_i]==sym) {
                        ...
                        if(curr==count-2&&buff_j-count+1>-1&&buff_i+count-1<SIZE_X&&
                                field[buff_j-count+1][buff_i+count-1]==sym&&field[buff_j-count+2][buff_i+count-2]==EMPTY_DOT)
                        {best_step=new int[]{buff_j-count+2,buff_i+count-2};return best_step;}//вариант х.ххх
                        if(curr==count-2&&buff_j+2<SIZE_Y&&buff_i-2>-1&&
                                field[buff_j+2][buff_i-2]==sym&&field[buff_j+1][buff_i-1]==EMPTY_DOT)
                        {best_step=new int[]{buff_j+1,buff_i-1};return best_step;}//вариант xxx.x
                        buff_i--;
                        buff_j++;
        ...
        for(int j=0; j<SIZE_Y;j++) {
            ...
            for (int i = 0; i < SIZE_X;i++ ) {
                ...
                if(curr==count-2&&i+2<SIZE_X&&field[j][i+2]==sym&&field[j][i+1]==EMPTY_DOT)
                {best_step=new int[]{j,i+1};return best_step;}//вариант ххх.х
            }
        }
        //вертикаль
        for(int i=0; i<SIZE_X;i++) {
            ...
            for (int j = 0; j < SIZE_Y; j++) {
                ...
                if(curr==count-2&&j+2<SIZE_Y&&field[j+2][i]==sym&&field[j+1][i]==EMPTY_DOT)
                {best_step=new int[]{j+1,i};return best_step;}//вариант ххх.х
            }}
        return best_step;
    }
    public static int[] VictoryIn2Step(char sym){
        int[]best_step={-1,-1};
            ...
            for (int j = 0; j < SIZE_Y;j++ ) {
                ...
                while(buff_i<SIZE_X&&buff_j<SIZE_Y){
                    ...
                    if (curr == count-2&&buff_j-count+1>-1&&buff_i-count+1>-1&&
                            field[buff_j-count+1][buff_i-count+1]==EMPTY_DOT&&
                            field[buff_j-count+2][buff_i-count+2]==EMPTY_DOT&&
                            buff_j+1<SIZE_Y&&buff_i+1<SIZE_X&&field[buff_j+1][buff_i+1]!=EMPTY_DOT&&sym!=PLAYER_DOT)
                    {best_step=new int[]{buff_j-count+2,buff_i-count+2};return best_step;};

                    if(curr==count-2&&buff_i-(count-2)>-1&&buff_i+2<SIZE_X&&buff_j-(count-2)>-1&&buff_j+2<SIZE_Y&&//xooo..
                            field[buff_j-count+2][buff_i-count+2]!=EMPTY_DOT&&
                            (field[buff_j+2][buff_i+2]==(EMPTY_DOT)||field[buff_j+2][buff_i+2]==(sym))&&
                            field[buff_j+1][buff_i+1]==EMPTY_DOT&&sym!=PLAYER_DOT)
                    { best_step=new int[]{buff_j+1,buff_i+1};return best_step;}
                    //при .х.хх.
                    if(count>3&&curr==count-3&&buff_i-(count-1)>-1&&buff_j-(count-1)>-1&&buff_i+1<SIZE_X&&buff_j+1<SIZE_Y&& field[buff_j-count+1][buff_i-count+1]==EMPTY_DOT
                            &&field[buff_j-count+2][buff_i-count+2]==sym&&field[buff_j-count+3][buff_i-count+3]==EMPTY_DOT&&field[buff_j+1][buff_i+1]==EMPTY_DOT)
                    { best_step=new int[]{buff_j-count+3,buff_i-count+3};return best_step;}

                    if(count>3&&curr==count-3&&buff_i-(count-3)>-1&&buff_i+3<SIZE_X&&buff_j-(count-3)>-1&&buff_j+3<SIZE_Y
                            && field[buff_j-count+3][buff_i-count+3]==EMPTY_DOT
                            &&field[buff_j+2][buff_i+2]==sym&&field[buff_j+1][buff_i+1]==EMPTY_DOT&&field[buff_j+3][buff_i+3]==EMPTY_DOT)
                    { best_step=new int[]{buff_j+1,buff_i+1};return best_step;}
                    buff_i++;
                    buff_j++;
                }
            ...
            for (int j = 0; j <SIZE_Y; j++) {
                ...
                while(buff_i>-1&&buff_j<SIZE_Y){
                    if(field[buff_j][buff_i]==sym) {
                        ...
                        if (curr == count - 2 && buff_j + 1 < SIZE_Y && buff_i - 1 > -1 &&
                                field[buff_j + 1][buff_i - 1] != EMPTY_DOT&&
                                buff_j-count+1>-1&&buff_i+count-1<SIZE_X&&
                                field[buff_j-count+1][buff_i+count-1]==EMPTY_DOT&&
                                field[buff_j-count+2][buff_i+count-2]==EMPTY_DOT&&sym!=PLAYER_DOT) {
                            best_step = new int[]{buff_j-count+2, buff_i+count-2};
                            return best_step;
                        }

                        if(curr==count-2&&buff_i+(count-2)<SIZE_X&&buff_i-2>-1&&buff_j-(count-2)>-1&&buff_j+2<SIZE_Y&&//xooo..
                                field[buff_j-count+2][buff_i+count-2]!=EMPTY_DOT&&
                                (field[buff_j+2][buff_i-2]==(EMPTY_DOT)||field[buff_j+2][buff_i-2]==(sym))&&
                                field[buff_j+1][buff_i-1]==EMPTY_DOT&&sym!=PLAYER_DOT)
                        { best_step=new int[]{buff_j+1,buff_i-1};return best_step;}
                        if(count>3&&curr==count-3&&buff_i+(count-1)<SIZE_X&&buff_j-(count-1)>-1&&buff_i-1>-1&&buff_j+1<SIZE_Y&& field[buff_j-count+1][buff_i+count-1]==EMPTY_DOT
                                &&field[buff_j-count+2][buff_i+count-2]==sym&&field[buff_j-count+3][buff_i+count-3]==EMPTY_DOT&&field[buff_j+1][buff_i-1]==EMPTY_DOT)
                        { best_step=new int[]{buff_j-count+3,buff_i+count-3};return best_step;}

                        if(count>3&&curr==count-3&&buff_i+(count-3)<SIZE_X&&buff_i-3>-1&&buff_j-(count-3)>-1&&buff_j+3<SIZE_Y
                                && field[buff_j-count+3][buff_i+count-3]==EMPTY_DOT
                                &&field[buff_j+2][buff_i-2]==sym&&field[buff_j+1][buff_i-1]==EMPTY_DOT&&field[buff_j+3][buff_i-3]==EMPTY_DOT)
                        { best_step=new int[]{buff_j+1,buff_i-1};return best_step;}
                        buff_i--;
        ...
        for(int j=0; j<SIZE_Y;j++) {
            ...
            for (int i = 0; i < SIZE_X;i++ ) {
                ...
                if(curr==count-2&&i-(count-1)>-1&&i+1<SIZE_X&&//.ххх.
                        field[j][i-count+1]==EMPTY_DOT&&
                        field[j][i-count+2]==EMPTY_DOT&&field[j][i+1]!=EMPTY_DOT&&sym!=PLAYER_DOT)
                { best_step=new int[]{j,i-count+2};return best_step;}
                if(curr==count-2&&i-(count-2)>-1&&i+2<SIZE_X&&//xooo..
                        field[j][i-count+2]!=EMPTY_DOT&&
                        (field[j][i+2]==(EMPTY_DOT)||field[j][i+2]==(sym))&&
                        field[j][i+1]==EMPTY_DOT&&sym!=PLAYER_DOT)
                { best_step=new int[]{j,i+1};return best_step;}
                if(count>3&&curr==count-3&&i-(count-1)>-1&&i+1<SIZE_X&& field[j][i-count+1]==EMPTY_DOT
                        &&field[j][i-count+2]==sym&&field[j][i-count+3]==EMPTY_DOT&&field[j][i+1]==EMPTY_DOT)
                { best_step=new int[]{j,i-count+3};return best_step;}

                if(count>3&&curr==count-3&&i-(count-3)>-1&&i+3<SIZE_X&& field[j][i-count+3]==EMPTY_DOT
                        &&field[j][i+2]==sym&&field[j][i+1]==EMPTY_DOT&&field[j][i+3]==EMPTY_DOT)
                { best_step=new int[]{j,i+1};return best_step;}
            }
        }
        //вертикаль
        for(int i=0; i<SIZE_X;i++) {
            ...
            for (int j = 0; j < SIZE_Y; j++) {
                ...
                {  best_step=new int[]{j+1,i};return best_step;}
                if(curr==count-2&&j-(count-1)>-1&&j+1<SIZE_Y&&
                        field[j-count+1][i]==EMPTY_DOT&&
                        field[j-count+2][i]==EMPTY_DOT&&field[j+1][i]!=EMPTY_DOT&&sym!=PLAYER_DOT)
                {  best_step=new int[]{j-count+2,i};return best_step;}

                if(curr==count-2&&j-(count-2)>-1&&j+2<SIZE_X&&//xooo.
                        field[j-count+2][i]!=EMPTY_DOT&&
                        (field[j+2][i]==(EMPTY_DOT)||field[j+2][i]==(sym))&&
                        field[j+1][i]==EMPTY_DOT&&sym!=PLAYER_DOT)
                { best_step=new int[]{j+1,i};return best_step;}
                if(count>3&&curr==count-3&&j-(count-1)>-1&&j+1<SIZE_X&& field[j-count+1][i]==EMPTY_DOT
                        &&field[j-count+2][i]==sym&&field[j-count+3][i]==EMPTY_DOT&&field[j+1][i]==EMPTY_DOT)
                { best_step=new int[]{j-count+3,i};return best_step;}

                if(count>3&&curr==count-3&&j-(count-3)>-1&&j+3<SIZE_Y&& field[j-count+3][i]==EMPTY_DOT
                        &&field[j+2][i]==sym&&field[j+1][i]==EMPTY_DOT&&field[j+3][i]==EMPTY_DOT)
                { best_step=new int[]{j+1,i};return best_step;}
            }}
        return best_step;
    }
    public static int[] VictoryIn3Step(char sym){
        int[]best_step={-1,-1};
    ...
    public static int[] findWinStep(char sym,int before){
        ...
        switch (before){
            case 1:{
               best_step=VictoryIn1Step(sym);
                return best_step;
            }
            case 2:{
                best_step=VictoryIn2Step(sym);
                return best_step;
            }
            case 3:{
                best_step=VictoryIn3Step(sym);
                return best_step;
            }
            }
        return best_step;
